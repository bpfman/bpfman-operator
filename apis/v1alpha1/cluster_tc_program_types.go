...
type ClTcProgramInfo struct {
	...
	//
	// The hook point for a TC program is a network interface (or device). The
	...
	// node. Optionally, the TC program can also be installed into a set of
	// network namespaces.
	// +optional
	// +kubebuilder:default:={}
...
type ClTcAttachInfo struct {
	...
	// Egress, that the TC program should attach to for a given network device.
