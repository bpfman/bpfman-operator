...
func (r *XdpProgramReconciler) getNodeSelector() *metav1.LabelSelector {
	return &r.currentXdpProgram.Spec.NodeSelector
}
...
func (r *XdpProgramReconciler) getExpectedBpfPrograms(ctx context.Context) (*bpfmaniov1alpha1.BpfProgramList, error) {
	...
	if r.currentXdpProgram.Spec.AttachPoints[0].Containers != nil {
		...
		containerInfo, err := r.Containers.GetContainers(
			ctx,
			r.currentXdpProgram.Spec.AttachPoints[0].Containers.Namespace,
			r.currentXdpProgram.Spec.AttachPoints[0].Containers.Pods,
			r.currentXdpProgram.Spec.AttachPoints[0].Containers.ContainerNames,
			r.Logger,
		)
		if err != nil {
