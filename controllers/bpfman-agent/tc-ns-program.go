...
func (r *TcNsProgramReconciler) getExpectedBpfPrograms(ctx context.Context) (*bpfmaniov1alpha1.BpfNsProgramList, error) {
	...
	if r.currentTcNsProgram.Spec.Containers != nil {

		// There is a container selector, so see if there are any matching
		// containers on this node.
		containerInfo, err := getContainers(ctx, r.currentTcNsProgram.Spec.Containers, r.NodeName, r.Logger)
		if err != nil {
		...
		for _, iface := range r.interfaces {
			attachPoint := iface + "-" + r.currentTcNsProgram.Spec.Direction
			annotations := map[string]string{internal.TcNsProgramInterface: iface}
