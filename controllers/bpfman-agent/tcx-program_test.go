...
func TestTcxProgramControllerCreate(t *testing.T) {
	var (
		name            = "fakeTcxProgram"
		namespace       = "bpfman"
	...
	// A TcxProgram object with metadata and spec.
	tcx := &bpfmaniov1alpha1.TcxProgram{
		...
		Spec: bpfmaniov1alpha1.TcxProgramSpec{
			...
			TcxProgramInfo: bpfmaniov1alpha1.TcxProgramInfo{
	...
	s.AddKnownTypes(bpfmaniov1alpha1.SchemeGroupVersion, &bpfmaniov1alpha1.TcxProgramList{})
	...
	cl := fake.NewClientBuilder().WithStatusSubresource(tcx).WithStatusSubresource(&bpfmaniov1alpha1.BpfProgram{}).WithRuntimeObjects(objs...).Build()
	...
	r := &TcxProgramReconciler{ClusterProgramReconciler: cpr, ourNode: fakeNode}
	...
	err = rc.getBpfProgram(ctx, r, name, appProgramId, attachPoint, bpfProg)
...
func TestTcxProgramControllerCreateMultiIntf(t *testing.T) {
	var (
		name            = "fakeTcProgram"
		namespace       = "bpfman"
		bytecodePath    = "/tmp/hello.o"
	...
	err = rc.getBpfProgram(ctx, r, name, appProgramId0, attachPoint0, bpfProgEth0)
	require.NoError(t, err)
