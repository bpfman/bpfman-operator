...
type ProgramReconciler interface {
	...
	getAttachId() *uint32
	setProgramAttachStatus(status bpfmaniov1alpha1.ProgramAttachStatus)
	getProgramAttachStatus() bpfmaniov1alpha1.ProgramAttachStatus
...
func (r *ReconcilerCommon) reconcileBpfAttachment(ctx context.Context, rec ProgramReconciler) (bool, error) {
	...
	case false:
		...
		case true:
			...
			if err := bpfmanagentinternal.DetachBpfmanProgram(ctx, r.BpfmanClient, *rec.getAttachId()); err != nil {
				...
				rec.setCurrentAttachPointStatus(bpfmaniov1alpha1.ApAttachNotAttached)
			}
		case false:
	...
	remove := !rec.shouldAttach() && rec.getCurrentAttachPointStatus() == bpfmaniov1alpha1.ApAttachNotAttached
	return remove, nil
}

func isAttachSuccess(shouldAttach bool, status bpfmaniov1alpha1.AttachPointStatus) bool {
