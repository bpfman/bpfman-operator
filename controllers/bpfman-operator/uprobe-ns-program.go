		...
		if errors.IsNotFound(err) {
			...
			if err := r.Get(ctx, types.NamespacedName{Namespace: corev1.NamespaceAll, Name: ownerRef.Name}, uprobeProgram); err != nil {
				if errors.IsNotFound(err) {
					...
					r.Logger.Error(err, "failed getting UprobeNsProgram Object from ownerRef", "Name", req.NamespacedName)
				}
				return ctrl.Result{}, nil
			}

		} else {
			r.Logger.Error(err, "failed getting UprobeNsProgram Object", "Name", req.NamespacedName)
			return ctrl.Result{}, nil
		}
	}
...
func (r *UprobeNsProgramReconciler) updateStatus(ctx context.Context, _namespace string, name string, cond bpfmaniov1alpha1.ProgramConditionType, message string) (ctrl.Result, error) {
