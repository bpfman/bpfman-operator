	...
	if err := r.Get(ctx, req.NamespacedName, tcxProgram); err != nil {
		...
		if errors.IsNotFound(err) {
			...
			if err := r.Get(ctx, req.NamespacedName, bpfProgram); err != nil {
				if errors.IsNotFound(err) {
					r.Logger.V(1).Info("BpfNsProgram not found stale reconcile, exiting", "Name", req.NamespacedName)
				} else {
					r.Logger.Error(err, "failed getting BpfNsProgram Object", "Name", req.NamespacedName)
				}
			...
			if err := r.Get(ctx, types.NamespacedName{Namespace: corev1.NamespaceAll, Name: ownerRef.Name}, tcxProgram); err != nil {
				if errors.IsNotFound(err) {
					r.Logger.Info("TcxNsProgram from ownerRef not found stale reconcile exiting", "Name", req.NamespacedName)
				} else {
					r.Logger.Error(err, "failed getting TcxNsProgram Object from ownerRef", "Name", req.NamespacedName)
				}
				return ctrl.Result{}, nil
			}

		} else {
			r.Logger.Error(err, "failed getting TcxNsProgram Object", "Name", req.NamespacedName)
			return ctrl.Result{}, nil
		}
	}
...
func (r *TcxNsProgramReconciler) updateStatus(ctx context.Context, _namespace string, name string, cond bpfmaniov1alpha1.ProgramConditionType, message string) (ctrl.Result, error) {
