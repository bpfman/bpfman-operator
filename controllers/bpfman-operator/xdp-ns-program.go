	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			builder.WithPredicates(predicate.And(
				...
				internal.BpfNsProgramTypePredicate(internal.XdpNsString)),
			),
		).
		Complete(r)
}
...
func (r *XdpNsProgramReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	r.Logger.Info("bpfman-operator enter: xdp-ns", "Name", req.NamespacedName.Name)
	...
	if err := r.Get(ctx, req.NamespacedName, xdpNsProgram); err != nil {
		...
		if errors.IsNotFound(err) {
			...
			if err := r.Get(ctx, types.NamespacedName{Namespace: req.NamespacedName.Namespace, Name: ownerRef.Name}, xdpNsProgram); err != nil {
				if errors.IsNotFound(err) {
					r.Logger.Info("xdpNsProgram from ownerRef not found stale reconcile exiting", "Name", req.NamespacedName)
				} else {
					r.Logger.Error(err, "failed getting XdpNsProgram Object from ownerRef", "NAme", req.NamespacedName)
				}
				return ctrl.Result{}, nil
			}

		} else {
			r.Logger.Error(err, "failed getting XdpNsProgram Object", "Name", req.NamespacedName)
			return ctrl.Result{}, nil
		}
	}
...
func (r *XdpNsProgramReconciler) updateStatus(ctx context.Context, namespace string, name string, cond bpfmaniov1alpha1.ProgramConditionType, message string) (ctrl.Result, error) {
