	...
	if err := r.Get(ctx, req.NamespacedName, tcProgram); err != nil {
		...
		if errors.IsNotFound(err) {
			...
			if err := r.Get(ctx, types.NamespacedName{Namespace: corev1.NamespaceAll, Name: ownerRef.Name}, tcProgram); err != nil {
				if errors.IsNotFound(err) {
					r.Logger.Info("TcNsProgram from ownerRef not found stale reconcile exiting", "Name", req.NamespacedName)
				} else {
					r.Logger.Error(err, "failed getting TcNsProgram Object from ownerRef", "Name", req.NamespacedName)
				}
				return ctrl.Result{}, nil
			}

		} else {
			r.Logger.Error(err, "failed getting TcNsProgram Object", "Name", req.NamespacedName)
			return ctrl.Result{}, nil
		}
	}
...
func (r *TcNsProgramReconciler) updateStatus(ctx context.Context, _namespace string, name string, cond bpfmaniov1alpha1.ProgramConditionType, message string) (ctrl.Result, error) {
